...
export default function ProductEditPage() {
    ...
    useEffect(() => {
        ...
        Promise.all([
            ...
            getCoverages(),
            getRiders(),
        ]).then(([productRes, coverages, riders]) => {
            ...
            setSelCoverages(covMap);

            // 기존 특약 pre-fill
            setSelRiders(new Set((p.riders ?? []).map((r: any) => r.riderId)));

            // 기존 할인·할증 pre-fill (rate는 소수 → %)
            setAdjustments((p.adjustments ?? []).map((a: any) => ({
                itemName:     a.itemName ?? "",
    ...
    const handleSubmit = async () => {
        ...
        try {
            await updateProduct(pid, {
                ...
                coverages: Object.entries(selCoverages).map(([cid, c], idx) => ({
                    coverageMasterId: Number(cid),
                    basePremium: Number(c.basePremium) || 0,
                    mandatory: c.mandatory,
                    sortOrder: idx,
                })),
                riders: Array.from(selRiders).map((rid, idx) => ({
    ...
    if (error && initialLoading) return <p className="text-red-500 text-sm p-6">{error}</p>;
    ...
    return (
        <div className="max-w-3xl">
            ...
            <div className="flex items-center gap-3 mb-6">
                ...
                <span className="text-sm text-gray-400 font-mono">{info.productCode}</span>
            </div>
